...
class StandardLabelToBoundary:
    def __init__(self, mode='thick', **kwargs):
        self.mode = mode

    def __call__(self, m):
        return find_boundaries(m, mode=self.mode)
...
class Standardize:
    ...
    def __init__(self, eps=1e-10, mean=None, std=None, channelwise=False, global_stats=False, **kwargs):
        self.global_stats = global_stats
        self.mean = mean
        self.std = std
        self.eps = eps
        self.channelwise = channelwise
        # check if external mean and std is provided and use them instead of recomputing it for every patch
        if global_stats:
...
class GaussianBlur3D:
    def __init__(self, sigma=[.1, 2.], execution_probability=0.5, channelwise=False, **kwargs):
        self.sigma = sigma
        self.execution_probability = execution_probability
        self.channelwise = channelwise
    ...
    def __call__(self, x):
        if random.random() < self.execution_probability:
            sigma = random.uniform(self.sigma[0], self.sigma[1])
            if self.channelwise:
                channels = [gaussian(c, sigma=sigma) for c in x]
...
def get_augmentation(aug_name, dataset_name, seed, phase, **kwargs):
    ...
    # train time
    if aug_name == 'standard':
        if dataset_name == 'raw':
            return Compose([
                Standardize(mean=mean, std=std),
                RandomFlip(random_state),
                RandomRotate90(random_state),
                RandomRotate(random_state, 45, order=3),
                ElasticDeformation(random_state, spline_order=3, execution_probability=0.2),
                AdditiveGaussianNoise(random_state, scale=(0.1, 2.0), execution_probability=0.1),
                AdditivePoissonNoise(random_state, execution_probability=0.1),
                ToTensor(expand_dims=True)
            ])
        else:
            return Compose([
                RandomFlip(random_state),
                RandomRotate90(random_state),
                RandomRotate(random_state, 45, order=0),
                ElasticDeformation(random_state, spline_order=3, execution_probability=0.2),
        ...
        if dataset_name == 'raw':
            return Compose([
                ...
                AdditiveGaussianNoise(random_state, execution_probability=0.2),
                AdditivePoissonNoise(random_state, execution_probability=0.2),
                ToTensor(expand_dims=True)
            ])
        else:
            return Compose([
        ...
        raise ValueError(f'Unsupported augmentation pipeline: {aug_name}')
